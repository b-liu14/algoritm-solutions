...
public:
    int findMaxForm(vector<string>& strs, int m, int n) {
        ...
        for (int idx_str = 1; idx_str <= strs.size(); idx_str++) {
            // count number of 0 and 1
            int nc[2] = {0, 0};
            for (auto c : strs[idx_str - 1]) {
                nc[c - '0']++;
            }
            for (int n0 = m; n0 >= nc[0]; n0--) {
                for (int n1 = n; n1 >= nc[1]; n1--) {
                    dp[n0][n1] = max(dp[n0][n1], dp[n0 - nc[0]][n1 - nc[1]] + 1);
                }
            }
        }
        return dp.back().back();
